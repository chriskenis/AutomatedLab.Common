...
foreach ($computer in $ComputerName) {
	If (Test-Connection $computer -Quiet -count 2 -EA SilentlyContinue){
		if ($regKey = [Microsoft.Win32.RegistryKey]::OpenRemoteBaseKey('LocalMachine', $computer)) {
			if ($netRegKey = $regKey.OpenSubKey("$dotNetRegistry")) {
				foreach ($versionKeyName in $netRegKey.GetSubKeyNames()) {
					if ($versionKeyName -match '^v[123]') {
						...
						$NetFrameworkVersions +=  New-Object -TypeName PSObject -Property ([ordered]@{
			...
			if ($net4RegKey = $regKey.OpenSubKey("$dotNet4Registry")) {
				if (-not ($net4Release = $net4RegKey.GetValue('Release'))) { $net4Release = 30319 }
				$NetFrameworkVersions +=  New-Object -TypeName PSObject -Property ([ordered]@{
						ComputerName = $Computer
						Build        = $net4Release
	...
	Else { Write-Error "unable to connect to $($computer)" }
	}
}
